...
// BestSolutionSoFar: The optimal chess constraint solver after exhaustive exploration
// Represents the performance ceiling achieved through 6 phases of optimization attempts
case class BestSolutionSoFar(m: Int, n: Int, pieces: InitialPieces) {
  ...
  private val hashCache = new ConcurrentHashMap[Set[PieceAtSlot], String]()
  ...
  private def printStatistics(): Unit = {
    ...
    println(f"BestSolutionSoFar Statistics:")
  ...
  private def canonicalForm(disposition: Set[PieceAtSlot]): String = {
    Option(hashCache.get(disposition)) match {
      case Some(cached) =>
        cacheHits += 1
        cached
      case None =>
        ...
        if (hashCache.size < 10000) { // Prevent unbounded growth
          hashCache.put(disposition, canonical)
        }
